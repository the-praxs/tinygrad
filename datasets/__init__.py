...
def fetch_mnist():
  ...
  X_train = (
      parse(f"{os.path.dirname(__file__)}/mnist/train-images-idx3-ubyte.gz")
      [0x10:].reshape((-1, 28 * 28)).astype(np.float32))
  Y_train = parse(
      f"{os.path.dirname(__file__)}/mnist/train-labels-idx1-ubyte.gz")[8:]
  X_test = (
      parse(f"{os.path.dirname(__file__)}/mnist/t10k-images-idx3-ubyte.gz")
      [0x10:].reshape((-1, 28 * 28)).astype(np.float32))
  Y_test = parse(
      f"{os.path.dirname(__file__)}/mnist/t10k-labels-idx1-ubyte.gz")[8:]
...
def fetch_cifar(train=True):
  ...
  fn = f"{os.path.dirname(__file__)}/cifar-10-python.tar.gz"
