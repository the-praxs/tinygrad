...
class Token(NamedTuple):
  ...
  def render(self, with_type=False):
    ...
    return f"{self.name}." + "xyzw"[int(self.offset)]
...
def to_float4(x:List[Token]) -> Optional[Token]:
  ...
  if all_same([y.name for y in x]) and all(
      y.ltype == LocalTypes.float4 and y.offset == i for i, y in enumerate(x)):
...
class Linearizer:
  ...
  def global_store(self, i, idxs:List[Variable], store:List[Token], ssa) -> None:
    ...
    if (should_upcast := self.supports_float4 and
        (self.bufs[i].dtype not in (dtypes.float16, dtypes.int8, dtypes.uint8))
        and len(self.float4_axis(i)) == 1):
      store_offset_new = {
          k: Token(out_tokens[0].name, LocalTypes.float4)
          if all_same([x.name for x in out_tokens])
          and tuple(range(4)) == tuple(x.offset for x in out_tokens) else
          self.uop(UOps.CAST, ssa("alu", LocalTypes.float4), out_tokens)
  ...
  def simplify_merge_adjacent(self):
    ...
    for i in range(1, len(shapes[0])):
      can_merge = [((strides[j][i] != 0
                     and rets[j][-1][1] == shapes[j][i] * strides[j][i])
  ...
  def hand_coded_optimizations(self):
    ...
    if not self.float4_axis(0) and self.first_reduce <= 2 and self.first_reduce + 1 <= self.shape_len and prod(self.sts[0].shape[:self.first_reduce]) <= 2048:
      ...
      for sz in (([256, 16]) if prod(self.sts[0].shape[:self.first_reduce]) <= 32 else [16]):
        if all(st.shape[self.first_reduce] %
               sz == 0 or st.shape[self.first_reduce] == 1 for st in self.sts):
    ...
    while prod(self.sts[0].shape[:self.first_reduce]) >= 1024:
      xb_choices = [(
          ...
          upcast_amount,
                    ...
                    if self.full_shape[axis] % upcast_amount == 0 and any(
                        self.sts[buf_index].views[-1].strides[axis] == 0 and all(
                            x[1] != 0 for x in self.upcasted_axis(buf_index))
                        for buf_index in range(len(self.sts)))]
      if not len(xb_choices):
        break
