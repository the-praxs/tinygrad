...
class Interpreted:
  ...
  def exec_ast(self, ast:LazyOp, output=None, context=None, **kwargs):
    ...
    if DEBUG >= 3:
      print(
          f"*** {'exec' if created_context else '    '} {GlobalCounters.mem_used / 1000000000.0:5.2f} GB {(time.perf_counter() - st) * 1000.0:7.2f} ms op: {ast.op:20s} out({ret.dtype.name}): {str(ret._buf.shape) if hasattr(ret._buf, 'shape') else str(len(ret._buf)):30s} in({len(srcs)}):",
          list({
    ...
    if output is None or output.output_buffer is None:
      return ret
...
class ASTRunner:
  ...
  def optimize_local_size(self, rawbufs:List[RawBuffer], preserve_output=False, allow_cache=False) -> List[int]:
    ...
    local_dims = [[
        ...
        if x <= sz
    ] for sz in self.global_size]
    local_sizes = [list(x) for x in itertools.product(*local_dims) if prod(x) <= MAX_WORKGROUP] * 2  # try each valid size twice
    return min((self.timeit(rawbufs, local_size), local_size)
               for local_size in random.sample(local_sizes, len(local_sizes)))[1]
